function [polyhedrons, bbox_min, bbox_max] = Copy_of_voronoi_diagram_3D(params)
    ...
    % Step 2: Form polyhedrons for each superquadric and calculate bisectors
    num_clusters = length(clusters);
    polyhedrons = cell(num_clusters, 1);

    for i = 1:num_clusters
        cluster_i = clusters{i};
        polyhedron_cluster = []; % Initialize empty polyhedron for this cluster

        % Iterate over each superquadric in the cluster
        for idx_i = cluster_i
            B_superquadric = [];
            b_superquadric = [];

            % Calculate bisectors between this superquadric and the closest superquadric from each other cluster
            for j = 1:num_clusters
                if i ~= j
                    closest_superquadric = find_closest_superquadric(params, idx_i, clusters{j});

                    % Calculate bisecting plane for the closest pair
                    [u1_opt, v1_opt, u2_opt, v2_opt] = find_closest_points(params{idx_i}, params{closest_superquadric});
                    [X1_opt, Y1_opt, Z1_opt] = superquadric_point(params{idx_i}, u1_opt, v1_opt);
                    [X2_opt, Y2_opt, Z2_opt] = superquadric_point(params{closest_superquadric}, u2_opt, v2_opt);
                    ...
                    center_i = params{idx_i}(9:11);
                    if dot(normal, center_i - midpoint) < 0
                        B_superquadric = [B_superquadric; normal];
                        b_superquadric = [b_superquadric; dot(normal, midpoint)];
                    else
                        B_superquadric = [B_superquadric; -normal];
                        b_superquadric = [b_superquadric; -dot(normal, midpoint)];
                    end
            ...
            % Bounding box inequalities for each superquadric
            l = bbox_min';
            u = bbox_max';

            % Create a polyhedron for this superquadric
            rep.B = B_superquadric;
            rep.b = b_superquadric;
            rep.l = l;
            rep.u = u;
            poly_superquadric = polyh(rep, 'h');

            % Union with the existing cluster polyhedron using union(A U B)
            if isempty(polyhedron_cluster)
                polyhedron_cluster = poly_superquadric;
            else
                polyhedron_cluster = polyhedron_cluster | poly_superquadric;
            end
        end

        % Store the polyhedron for this cluster
        polyhedrons{i} = polyhedron_cluster;
    end
...
function closest_superquadric = find_closest_superquadric(params, superquadric_idx, cluster)
    % Find the closest superquadric to the given superquadric from the other cluster
    min_distance = Inf;
    closest_superquadric = cluster(1);
    for j = cluster
        [~, ~, ~, ~, dist] = find_closest_points(params{superquadric_idx}, params{j});
        if dist < min_distance
            min_distance = dist;
            closest_superquadric = j;
        end
    end
end
