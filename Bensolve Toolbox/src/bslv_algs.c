...
int alg(soltype *const sol, const vlptype *vlp, const opttype *opt)
{
	...
	else
	{
		...
		*/
		...
		{
			if (opt->message_level >= 1) printf("VLP is totally unbounded, there is no solution\n");
			return 1;
		}
		if (sol->status == VLP_NOVERTEX)
		{
			if (opt->message_level >= 1) printf("upper image of VLP has no vertex (this case is not covered by this version)\n");
			return 1;
		}
		/*
		* phase 1 
	...
	*/
	...
	if (opt->message_level >= 1)
	{
		...
		{
			if(vlp->q==1)
				printf("VLP with (q=1) is not bounded\n");
			else
			{
				if (opt->bounded==1)
...
void poly_output(poly_args *args, const opttype *opt, int stdout_at_msg_lev, pre_img_type pre_img, swap_type swap, const char *str1, const char *str2, const char *str3)
{
  ...
  //distinguishing between cone or sol output
  int cone_out=0;
  if(!strcmp(str1,CONE_P_STR))
    cone_out=9;
...
void phase0(soltype *const sol, const vlptype *vlp, const opttype *opt)
{
	...
	for (size_t i = 0; i < vlp->q - 1; i++)
	{
		...
		if (opt->message_level >= 3) printf("solve lp\n");
		{
			lp_status_type stat =lp_solve(0);
			assert(stat == LP_OPTIMAL);
		}
...
void phase2_primal(soltype *const sol, const vlptype *vlp, const opttype *opt)
{
	...
	if (opt->solution == PRE_IMG_ON)
		init_image (&upper_image, vlp->q, vlp->n, vlp->m+vlp->n+vlp->q, &lowerV2upperH);
	else
	{
...

	{
		...
		for (lp_idx j = 0; j < sol->r; j++)
		{
			...
			if (lp_status != LP_OPTIMAL)
			{
				...
				else if (lp_status == LP_UNBOUNDED)
					sol->status = VLP_UNBOUNDED;
				else
					sol->status = VLP_UNEXPECTED_STATUS;
				break;
			}
			if (opt->solution == PRE_IMG_ON)
			{
				lp_dual_solution_rows (0,upper_image.val_primg_dl,1,vlp->m,(vlp->optdir==1?1:-1)); // u rows
				lp_dual_solution_cols (0,upper_image.val_primg_dl+vlp->m,1,vlp->n,(vlp->optdir==1?1:-1)); // u cols
				for (size_t k=0;k<vlp->q;k++) 
					upper_image.val_primg_dl[vlp->m+vlp->n+k]=(sol->c_dir==C_DIR_POS?1:-1)*upper_image.val[k]; // w
				if (vlp->q==1)
					lp_primal_solution_cols (0, upper_image.primal.data_primg,1,vlp->n, 1);
			}
		...
		boundlist_free(rows);
		poly__intl_apprx (&upper_image);
		if (sol->status == VLP_INFEASIBLE || sol->status == VLP_UNBOUNDED || sol->status == VLP_UNEXPECTED_STATUS)
		{
			poly__kill (&upper_image);
			return;
		}
	...
	if (vlp->q>1)
	{
		...
		for(;;)
		{
			if (opt->globalsolve)
			{
				if (poly__get_vrtx_global (&upper_image,sol->c_dir==C_DIR_POS?1:-1))
					break;
			}
			else
			{
			...
			if (z > opt->eps_benson_phase2)
			{
				...
				if (opt->solution == PRE_IMG_ON)
				{
					...
					lp_dual_solution_cols (0, upper_image.val_primg_dl+vlp->m,1,vlp->n, (vlp->optdir==1?1:-1)); // u cols
					for (size_t k=0;k<vlp->q;k++)
						upper_image.val_primg_dl[vlp->m+vlp->n+k]=(sol->c_dir==C_DIR_POS?1:-1)*ww[k]; // w
				}
				poly__add_vrtx (&upper_image);
			}
			else
			{
				...
				if(opt->globalsolve==1)
				{	// write solution in corresponding output field and break
					mxArray *sol = mxCreateDoubleMatrix(1,vlp->n,mxREAL);
					mxSetField (global_sol, 0, "qc_sol",sol);
					double *out = mxGetPr(sol);
					lp_primal_solution_cols (0,out,1,vlp->n,1);
					break;
				}
			}
		}
...
void phase2_dual(soltype *const sol, const vlptype *vlp, const opttype *opt)
{
	...
	if (opt->solution == PRE_IMG_ON)
		init_image (&lower_image, vlp->q, vlp->m+vlp->n+vlp->q, vlp->n, &upperV2lowerH);
	else
		init_image (&lower_image, vlp->q, 0, 0, &upperV2lowerH);
	
	poly_args upper_image_globalsolve;
	if (opt->globalsolve==1) //  upper image
		init_image (&upper_image_globalsolve, vlp->q, 0, 0, &lowerV2upperH);	
	 ...
	 */
	{
		...
		lp_idx r=opt->globalsolve?sol->r+1:1;
		for (lp_idx k=0; k<r; k++)
		{
			if (k==0)
			{
			...
			if (lp_status != LP_OPTIMAL)
			{
				list1d_free(obj);
				if (opt->globalsolve)
					poly__kill(&upper_image_globalsolve);
				poly__kill (&lower_image);
				if (lp_status == LP_INFEASIBLE)
					sol->status = VLP_INFEASIBLE;
				else if (lp_status == LP_UNBOUNDED)
			...
			{	
				...
				if (vlp->q==1)
				{
					lp_dual_solution_rows (0, lower_image.primal.data_primg,1,vlp->m, (vlp->optdir==1?1:-1)); //u rows
					lp_dual_solution_cols (0, lower_image.primal.data_primg+vlp->m,1,vlp->n, (vlp->optdir==1?1:-1)); //u cols
					lower_image.primal.data_primg[vlp->m+vlp->n]=(sol->c_dir==C_DIR_POS?1:-1); //w
				}
			}	
			poly__add_vrtx(&lower_image);
			if(opt->globalsolve)
			{
				upper_image_globalsolve.ideal=0;
				for(lp_idx i=0; i<vlp->q-1; i++)
					upper_image_globalsolve.val[i]=w[i];
				upper_image_globalsolve.val[vlp->q-1]=lp_obj_val(0);
				poly__add_vrtx(&upper_image_globalsolve);
			}
			/*
	...
	if(opt->globalsolve==1)
		poly__intl_apprx (&upper_image_globalsolve);
	...
	if (vlp->q>1)
	{
		...
		for(;;)
		{
			if(opt->globalsolve)
			{
				double cfv, min_cfv1 = DBL_MAX, min_cfv2 = DBL_MAX;
				size_t min_idx = -1;
				poly__get_vrtx_global(&upper_image_globalsolve, sol->c_dir==C_DIR_POS?1:-1);
				for (lower_image.idx=0; lower_image.idx<lower_image.primal.cnt; lower_image.idx++)
				{	
					if (IS_ELEM(lower_image.primal.used, lower_image.idx)&&!IS_ELEM(lower_image.primal.ideal, lower_image.idx))
					{	
						// compute value of coupling function
						cfv=upper_image_globalsolve.val[vlp->q - 1] - *(VRTX_VAL(&lower_image.primal, lower_image.idx) + vlp->q - 1);
						for (size_t k=0; k<vlp->q - 1; k++)
							cfv+=(upper_image_globalsolve.val[k] - upper_image_globalsolve.val[vlp->q - 1] * sol->c[k])*(*(VRTX_VAL(&lower_image.primal, lower_image.idx)+k));
						if(!IS_ELEM(lower_image.primal.sltn, lower_image.idx) && cfv < min_cfv1)
						{
							min_cfv1=cfv;
							min_idx=lower_image.idx;
						}
						if (cfv < min_cfv2)
							min_cfv2=cfv;
					}
				...
				assert(min_idx !=-1);
				if (min_cfv2 < -2*opt->eps_benson_phase2)
				{
					lower_image.idx=min_idx;
					lower_image.ideal = IS_ELEM (lower_image.primal.ideal, lower_image.idx);
					for (size_t k=0; k < vlp->q; k++)
						lower_image.val[k]=*(VRTX_VAL(&lower_image.primal, lower_image.idx)+k);
				}
				else
				{
					// write solution in corresponding output field and break
					...
					for (size_t j=0;j<sol->r;j++)
					{
						rows->ub[j]=0;
						for (size_t k=0;k<vlp->q;k++)
							rows->ub[j]+=sol->R[k*sol->r+j]*upper_image_globalsolve.val[k];
					}
					lp_set_rows(0, rows);
					{
						lp_status_type stat =lp_solve(0);
			...
			// refine outer approximation of upper image
			if (opt->globalsolve) 
			{
				upper_image_globalsolve.ideal=0;
				for(lp_idx i=0; i<vlp->q-1; i++)
					upper_image_globalsolve.val[i]=obj->data[i];
				upper_image_globalsolve.val[vlp->q-1]=lp_obj_val(0);
				poly__add_vrtx(&upper_image_globalsolve);
			}
			...
			else
			{
				...
				if (opt->solution == PRE_IMG_ON)
				{
					lp_dual_solution_rows (0, lower_image.primal.data_primg+lower_image.primal.dim_primg*lower_image.idx,1,vlp->m, (vlp->optdir==1?1:-1)); //u
					lp_dual_solution_cols (0, (lower_image.primal.data_primg+lower_image.primal.dim_primg*lower_image.idx)+vlp->m,1,vlp->n, (vlp->optdir==1?1:-1)); //u
					for (size_t k=0; k<vlp->q; k++)
						lower_image.primal.data_primg[lower_image.primal.dim_primg*lower_image.idx + vlp->m + vlp->n + k]=(sol->c_dir==C_DIR_POS?1:-1)*obj->data[k]; //w
				}
			}
		}
		list1d_free(obj);
