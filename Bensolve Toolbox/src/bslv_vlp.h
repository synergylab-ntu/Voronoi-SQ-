...
typedef struct 
{
	int bounded;
	int plot;
	int globalsolve;						// Flag to indicate global optimization mode
	char filename[255+1];
...
typedef struct
{	
	char valid_bool_options[MAX_OPTION_STR_LNGTH+1];
	char valid_string_options[MAX_OPTION_STR_LNGTH+1];
	int n_bool_options;
	int n_string_options;	
	//int max_str_len;	
	int argv_dim;
	int argv_element_dim;
} mexopttype;
